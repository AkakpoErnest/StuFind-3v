...
export const connectWallet = async () => {
  if (typeof window !== "undefined" && window.ethereum) {
    ...
    throw new Error("MetaMask not found. Please install MetaMask to use Stufind.")
  }
}
...
export const createMarketItem = async (provider, itemData, imageFile) => {
  try {
    ...
    const metadataHash = await createNFTMetadata(
      {
        ...itemData,
        seller: await provider.getSigner().getAddress(),
      },
      imageHash,
    )
    ...
    const gasEstimate = await contract.estimateGas.createToken(
      tokenURI,
      ethers.utils.parseEther(itemData.price.toString()),
      itemData.category,
      itemData.condition,
...
export const getStudentProfile = async (provider, address) => {
  try {
    ...
    return {
      ...
      totalSales: profile[2].toNumber(),
      totalPurchases: profile[3].toNumber(),
      rating: profile[4].toNumber() / 100, // Convert back from basis points
      ratingCount: profile[5].toNumber(),
      isActive: profile[6],
    }
  } catch (error) {
...
export const getMarketStats = async (provider) => {
  try {
    ...
    return {
      totalItems: stats[0].toNumber(),
      totalSold: stats[1].toNumber(),
      totalVolume: ethers.utils.formatEther(stats[2]),
      averagePrice: ethers.utils.formatEther(stats[3]),
    }
  } catch (error) {
