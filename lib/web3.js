...
export const connectWallet = async () => {
  if (typeof window === "undefined" || !window.ethereum) {
    throw new Error("MetaMask not found or not in browser environment.")
  }
  try {
...
export const createMarketItem = async (provider, itemData, imageFile) => {
  try {
    ...
    const transaction = await contract.createToken(
      tokenURI,
      ethers.parseEther(itemData.price), // Use ethers.parseEther
      itemData.category,
      itemData.condition,
...
export const buyItem = async (provider, tokenId, price) => {
  try {
    ...
    const transaction = await contract.createEscrow(tokenId, {
      value: ethers.parseEther(price), // Use ethers.parseEther
    })
    await transaction.wait()
    return transaction
  } catch (error) {
...
export const fetchMarketItems = async (provider) => {
  try {
    ...
    return items.map((item) => ({
      ...
      category: item[5],
      condition: item[6],
      royaltyPercent: Number(item[7]) / 100, // Convert BigInt to Number
      creator: item[8],
